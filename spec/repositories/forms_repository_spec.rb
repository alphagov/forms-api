describe Repositories::FormsRepository do
  ...
  context "creating a new form" do
    it "creates a form" do
      result = subject.create("name", "submission_email", "org", true)
      ...
      expect(created_form[:published]).to eq(true)
    end
  end

  context "getting a form" do
    it "gets a form" do
      form_id = subject.create("name", "submission_email", "org", false)
    ...
    it "gets a form by org" do
      subject.create("name", "submission_email", "org", false)
      subject.create("name2", "submission_email", "org", false)
  ...
  context "updating a form" do
    it "updates a form" do
      form_id = subject.create("name", "submission_email", "org", false)
      update_result = subject.update(form_id, "name2", "submission_email2", "org2", false)
      form = subject.get(form_id)
  ...
  context "getting all forms" do
    it "gets all forms" do
      form_id1 = subject.create("name", "submission_email", "org", false)
      form_id2 = subject.create("name", "submission_email", "org", false)
