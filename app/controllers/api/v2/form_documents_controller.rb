class Api::V2::FormDocumentsController < ApplicationController
  ...
  def index
    ...
    response.set_header("X-total", paginated.total_count.to_s)
    response.set_header("X-offset", paginated.offset_value.to_s)
    response.set_header("X-limit", paginated.limit_value.to_s)
