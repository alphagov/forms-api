class Reports::SelectionQuestionService
  UsageStatistics = Struct.new(:unique_form_ids_set, :question_count, :optional_question_count) do
    def as_json(*)
      super.except("unique_form_ids_set").tap do |h|
        h["form_count"] = unique_form_ids_set.length
      end
    end
  end

  def live_form_statistics
    statistics = {
    ...
    all_selection_questions.all.map do |page|
      if page.answer_settings["only_one_option"] == "true"
        if page.answer_settings["selection_options"].length > 30
          statistics[:autocomplete].unique_form_ids_set.add(page.form_id)
          statistics[:autocomplete].question_count += 1
          statistics[:autocomplete].optional_question_count += 1 if page.is_optional
        else
          statistics[:radios].unique_form_ids_set.add(page.form_id)
          statistics[:radios].question_count += 1
          statistics[:radios].optional_question_count += 1 if page.is_optional
        end
      else
        statistics[:checkboxes].unique_form_ids_set.add(page.form_id)
        statistics[:checkboxes].question_count += 1
        statistics[:checkboxes].optional_question_count += 1 if page.is_optional
      end
    end
